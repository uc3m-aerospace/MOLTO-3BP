...
npoints = 900;%700%600%500
nodes = 900;%700%600%500
...
x0min = -R_GEO;%min([x0,0.99*x0,1.01*x0]);
x0max = R_GEO;%max([x0,0.99*x0,1.01*x0]);
y0min = -R_GEO;%min([y0,0.99*y0,1.01*y0]);
y0max = R_GEO;%max([y0,0.99*y0,1.01*y0]);
xdot0min = -4;%min([xdot0,0.5*xdot0,1.5*xdot0]);
xdot0max = 4;%max([xdot0,0.5*xdot0,1.5*xdot0]);
ydot0min = -4;%min([ydot0,0.5*ydot0,1.5*ydot0]);
ydot0max = 4;%max([ydot0,0.5*ydot0,1.5*ydot0]);
m0min = m00/MU;
m0max = m00/MU;
...
mfmin = 0;
mfmax = m0;
xfmin = (1-mu_EM)-10e6/DU; %min([xf,0.99*xf,1.01*xf]);
xfmax = (1-mu_EM)+10e6/DU;%max([xf,0.99*xf,1.01*xf]);
yfmin = -10e6/DU;%min([yf,0.99*yf,1.01*yf]);
yfmax = 10e6/DU;%max([yf,0.99*yf,1.01*yf]);
xdotfmin = -4;%min([xdotf,0.5*xdotf,2*xdotf]);
xdotfmax = 4;%max([xdotf,0.5*xdotf,2*xdotf]);
ydotfmin = -4;%min([ydotf,0.5*ydotf,2*ydotf]);
ydotfmax = 4;%max([ydotf,0.5*ydotf,2*ydotf]);
...
ymin = -5;
ymax = 5;
xdotmin = -4;%min([min(IG_states_n(3,:)),xdot0min,xdotfmin]);
xdotmax = 4;%max([max(IG_states_n(3,:)),xdot0max,xdotfmax]);
ydotmin = -4;%min([min(IG_states_n(4,:)),ydot0min,ydotfmin]);
ydotmax = 4;%max([max(IG_states_n(4,:)),ydot0max,ydotfmax]);
mmin = 0;
mmax = m0;
...
limits(iphase).event.min   = [(100 + R_m)*1000/DU;(499 + R_m)*1000/DU;-1e-20;-1e-20;0*sqrt((1-CONSTANTS.mu)/r0);R_GEO+1e-20];
limits(iphase).event.max   = [(500 + R_m)*1000/DU; (6000 + R_m)*1000/DU;1e-20;1e-20;0.5*sqrt((1-CONSTANTS.mu)/r0);R_GEO-1e-20];
...
%guess(iphase).control(:,1)    = solution.control(:,1);
%guess(iphase).control(:,2)    = solution.control(:,2);
guess(iphase).parameter       = [];

linkages = [];
setup.CONSTANTS = CONSTANTS;
setup.name  = 'mooncubesatProblem';
setup.funcs.cost = 'mooncubesatCostD';
setup.funcs.dae = 'mooncubesatDae';
setup.funcs.event = 'mooncubesateventfunD';
setup.limits = limits;
setup.guess = guess;
...
%set(gca,'ticklabelinterpreter','latex','fontsize',15)
%savefig(strcat([date 'h_moon.fig']))
