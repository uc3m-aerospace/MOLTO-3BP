...
phi_EM_0 = setup.CONSTANTS.phi_EM_0;
...
dOm4dx = x - (m_s.*cos(om_s.*t-phi_EM_0))./rho.^2 - (mu.*(mu + x - 1))./((mu + x - 1).^2 + y.^2).^(3/2) + ((2.*mu + 2.*x).*(mu - 1))./(2.*((mu + x).^2 + y.^2).^(3/2)) - (m_s.*(2.*x - 2.*rho.*cos(om_s.*t-phi_EM_0)))./(2.*((x - rho.*cos(om_s.*t-phi_EM_0)).^2 + (y - rho.*sin(om_s.*t-phi_EM_0)).^2).^(3/2));
dOm4dy = y - (m_s.*sin(om_s.*t-phi_EM_0))./rho.^2 - (mu.*y)./((mu + x - 1).^2 + y.^2).^(3/2) - (m_s.*(2.*y - 2.*rho.*sin(om_s.*t-phi_EM_0)))./(2.*((x - rho.*cos(om_s.*t-phi_EM_0)).^2 + (y - rho.*sin(om_s.*t-phi_EM_0)).^2).^(3/2)) + (y.*(mu - 1))./((mu + x).^2 + y.^2).^(3/2);
